...
// - Used _NET_WM_DESKTOP instead of _NET_WM_DESKTOP as window type 
// - Commented out lines that determined the area of the bar in order to refute influence on window maximizing
...
set_ewmh_atoms (void)
{
    ...
    for (monitor_t *mon = monhead; mon; mon = mon->next) {
        ...
        // Changed _DOCK to _DESKTOP
        xcb_change_property(c, XCB_PROP_MODE_REPLACE, mon->window, atom_list[NET_WM_WINDOW_TYPE], XCB_ATOM_ATOM, 32, 1, &atom_list[NET_WM_WINDOW_TYPE_DESKTOP]);
        ...
        // Lines are commented out in order that Windows are not influenced by dock dimensions
